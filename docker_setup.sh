...
docker cp includes/plugins/plugin.php $(docker ps | grep 33060 | sed -e "s/\s.*//g"):/plugins/plugin.php
docker cp includes/plugins/frames.php $(docker ps | grep 33060 | sed -e "s/\s.*//g"):/plugins/frames.php
...
#docker exec -i $(docker ps | grep 33060 | sed -e "s/\s.*//g") sh -c 'cat > /plugins/plugin.php' < includes/plugins/plugin.php
#docker exec -i $(docker ps | grep 33060 | sed -e "s/\s.*//g") sh -c 'cat > /plugins/frames.php' < includes/plugins/frames.php